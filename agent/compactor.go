...
type Compactor interface {
	...
	// typed trigger reason. An empty trigger means compaction is not needed.
	ShouldCompact(req CompactionRequest) (trigger CompactionTrigger, should bool)
}
...
type CompactionRequest struct {
	...
	// Budget carries the resolved context window, compaction threshold, and
	// bound prompt shape (system prompt, tools, format) used for the due-ness
	// estimate. A zero-value budget is valid when Force is set: assessDue
	// short-circuits on Force before touching the budget.
	Budget PromptBudget
}
...
// assessDue is the single due-ness decider shared by ShouldCompact and
// MaybeCompact. It prefers the request's pre-resolved Budget (built once per
// run by the Session); when Budget is unset — e.g. manual compaction or a
// caller that didn't build one — it falls back to deriving the threshold from
// the compactor's own config, preserving the former behavior. The estimate
// uses the budget's bound prompt shape when present, otherwise counts only the
// messages.
func (c *SimpleCompactor) assessDue(req CompactionRequest) (due bool, trigger CompactionTrigger) {
	if req.Force {
		return true, CompactionTriggerForce
	}
	threshold := req.Budget.Threshold()
	if threshold <= 0 {
		threshold = int(float64(c.contextWindowTokens(req.Options)) * c.threshold())
		if threshold <= 0 {
			return false, ""
		}
	}
	if req.Latest.PromptEvalCount > 0 && req.Latest.PromptEvalCount >= threshold {
		return true, CompactionTriggerPromptEval
	}
	if req.Budget.Estimate(req.Messages) >= threshold {
		return true, CompactionTriggerEstimate
	}
	return false, ""
}
...
// ShouldCompact reports whether compaction is due and the typed trigger.
// Satisfies the Compactor interface.
func (c *SimpleCompactor) ShouldCompact(req CompactionRequest) (CompactionTrigger, bool) {
	if c == nil {
		return "", false
	}
	due, trigger := c.assessDue(req)
	return trigger, due
}
...
// estimatePromptTokens approximates the token count of a chat prompt
// composed of a system prompt, messages, tools, and format. It mirrors the
// request payload the model would receive (marshal + chars/4 heuristic). It is
// decoupled from CompactionRequest so PromptBudget can call it with its bound
// prompt shape.
func estimatePromptTokens(systemPrompt string, messages []api.Message, tools api.Tools, format string) int {
	requestMessages := sanitizeMessagesForEstimate(messages)
	if strings.TrimSpace(systemPrompt) != "" {
		...
		requestMessages = append(requestMessages, sanitizeMessagesForEstimate(messages)...)
	}
	...
	if rawFormat, ok := compactionFormatForEstimate(format); ok {
	...
	total += estimateCompactionTokens(tools.String())
	total += estimateCompactionTokens(format)
	return total
}

func (s *Session) checkPreflightPromptBudget(budget PromptBudget, opts RunOptions, messages []api.Message) error {
	if budget.ContextWindow() <= 0 {
		return nil
	}
	estimated := budget.Estimate(messages)
	if estimated < budget.ContextWindow() {
		return nil
	}
	return fmt.Errorf("prompt is too large for the current context (~%d/%d tokens). Reduce the system prompt or message history, compact the conversation, or use a model with a larger context", estimated, budget.ContextWindow())
}

func (s *Session) checkPostCompactionPromptBudget(budget PromptBudget, opts RunOptions, messages []api.Message) error {
	if budget.ContextWindow() <= 0 {
		return nil
	}
	estimated := budget.Estimate(messages)
	...
	return fmt.Errorf("history is still too large after compaction (~%d/%d tokens). Start a fresh request, reduce the system prompt or history, or use a model with a larger context", estimated, budget.ContextWindow())
}
