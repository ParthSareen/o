...
import (
	...
	agentchat "github.com/ParthSareen/o/cmd/tui/chat"
...
func prepareAgentModel(cmd *cobra.Command, client *api.Client, opts *agentTUIOptions, thinkExplicit bool) (*api.ShowResponse, error) {
	...
	opts.Think, err = inferThinkingOption(info.Capabilities, opts.Think, thinkExplicit)
	if err != nil {
		return nil, err
	}
...
func agentDefaultSystemPromptWithWorkingDir(now time.Time, modelName string, workingDir string) string {
	...
	parts = append(parts,
		...
		"Follow-ups: every headless run prints its saved session ID on stderr as `session: <id>`; background-task logs carry the line and the [background task update] notice repeats it. Once a child has EXITED, continue its session without losing context: `o --resume-id <id> --headless --allow-all-tools \"follow-up\"` (the child's model is remembered). Never use bare --resume for this (it targets the most recent session, usually this one) and never resume a session while another process is still running it. For long child tasks, run them with bash background=true.",
	)
	if workingDir != "" {
